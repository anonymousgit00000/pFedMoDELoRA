...
def apply_lora_with_dynamic_scan(model, r=8, alpha=16, dropout=0.1, bias="none"):
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "MLR": # convex
            ...
            # args.model = torchvision.models.resnet34(pretrained=True, num_classes=args.num_classes).to(args.device)
            args.model = torchvision.models.resnet34(weights=torchvision.models.ResNet34_Weights.DEFAULT)
            in_features = args.model.fc.in_features
            args.model.fc = nn.Linear(in_features, args.num_classes)
            args.model = args.model.to(args.device)

        elif model_str == "ViT":
            # args.model = timm.create_model('vit_base_patch16_224', pretrained=True, num_classes=args.num_classes).to(args.device)
            # args.model = timm.create_model('vit_small_patch16_224', pretrained=True, num_classes=args.num_classes).to(args.device)
            args.model = timm.create_model('vit_small_patch16_224', pretrained=True, num_classes=args.num_classes).to(args.device)
            # args.model = timm.create_model('swin_base_patch4_window7_224', pretrained=True, num_classes=args.num_classes)
        ...
        if args.algorithm == "FedAvg":
            if hasattr(args.model, "fc"):   # ResNet, etc
                args.head = copy.deepcopy(args.model.fc)
                args.model.fc = nn.Identity()
            elif hasattr(args.model, "head"):  # ViT
                args.head = copy.deepcopy(args.model.head)
                args.model.head = nn.Identity()
            else:
                raise NotImplementedError("Model doesn't have fc or head.")

            args.model = BaseHeadSplit(args.model, args.head)
            # args.model.base = get_peft_model(args.model.base, lora_config)
            args.model.base = apply_lora_with_dynamic_scan(args.model.base)
            ...
            args.model = apply_lora_with_dynamic_scan(args.model)
            server = APPLE(args, i)
        ...
        # -------------------
        # 检查是否正确插入了LoRA
        has_lora_in_base = False
        has_lora_in_head = False

        if hasattr(args.model, 'base'):
            for name, _ in args.model.base.named_parameters():
                if 'lora_' in name:
                    has_lora_in_base = True
            for name, _ in args.model.head.named_parameters():
                if 'lora_' in name:
                    has_lora_in_head = True
        else:
            for name, _ in args.model.named_parameters():
                if 'lora_' in name:
                    has_lora_in_base = True
...
if __name__ == "__main__":
    ...
    # ViT 学习率
    # parser.add_argument('-lr', "--local_learning_rate", type=float, default=5e-5,
    #                     help="Local learning rate")
    parser.add_argument('-lr', "--local_learning_rate", type=float, default=5e-5,
                        help="Local learning rate")
    ...
    parser.add_argument('-ldg', "--learning_rate_decay_gamma", type=float, default=0.95)
    ...
    parser.add_argument('-sfn', "--save_folder_name", type=str, default='d01')
