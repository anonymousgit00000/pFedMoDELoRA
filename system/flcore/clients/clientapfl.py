...
class clientAPFL(Client):
    ...
    def train(self):
        ...
        # lora_params_main = filter(lambda p: p.requires_grad, self.model.parameters())
        # lora_params_main = list(lora_params_main)
        # self.optimizer = torch.optim.Adam(lora_params_main, lr=self.learning_rate)
        ...
        # lora_params_per = filter(lambda p: p.requires_grad, self.model.parameters())
        # lora_params_per = list(lora_params_per)
        # self.optimizer_per = torch.optim.Adam(lora_params_per, lr=self.learning_rate)

        for name, param in self.model.named_parameters():
            param.requires_grad = ("lora_" in name or "head" in name)
        for name, param in self.model_per.named_parameters():
            param.requires_grad = ("lora_" in name or "head" in name)

        lora_params_main = list(filter(lambda p: p.requires_grad, self.model.parameters()))
        lora_params_per = list(filter(lambda p: p.requires_grad, self.model_per.parameters()))
    ...
    def alpha_update(self):
        # print(f"[DEBUG] alpha type: {type(self.alpha)}, value: {self.alpha}")
        grad_alpha = 0
        for l_params, p_params in zip(self.model.parameters(), self.model_per.parameters()):
            if l_params.grad is None or p_params.grad is None:
                continue
